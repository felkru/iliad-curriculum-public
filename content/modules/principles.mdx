...
teachingGuide: >-
  First, start with the lecture, which should take 1-1.5 hours. Ask for
  ...
  with intermediate discussions. For A.2, this should only be the case for the
  Solomonoff induction reading (Shane Legg’s thesis)


  For exercises:


  - Break the exercises into chunks depending on content and duration (aim for
  20-40 mins). Set a timer for how long you expect the chunk to take
  (underestimate is good), and assess people’s progress when the timer runs out
  and possibly/likely run the timer again. Explicitly tell the students they can
  ask for help at any time. After the chunk is up, ask questions of the class
  (“what answer did you get here? what pattern might that suggest?” etc), make
  some remarks about takeaways, etc. Repeat.
...
notesForFutureIterations: >
  - Making a comprehensive set of lecture notes that actually ties the readings
  together rather than just listing them would be nice. Or at a minimum just an
  annotated reading list  

  - Motivate better what the Solomonoff exercises are building toward  

  - Mathematically showing the exact decomposition of empirical risk into
  approximation, generalization, optimization in e.g. the ERM framework would be
  helpful, to make these categories less hand-wave-y  

  - Approximation theory and the curse of dimensionality should be covered today
  (e.g. the parameter count lower bound from
  [here](https://gwern.net/doc/cs/algorithm/1989-devore.pdf)), I just couldn’t
  find any good existing resources so this might need to be created  

  - This covered a lot of ground in one day and people felt a bit rushed  

  - Preparing explicit discussion prompts ahead of time for readings could be a
  good idea  

  - Small group discussions (\~4 people) generally work better than full class
  discussions  

  - Learning goals should be made more clear. People generally did not realize
  that the end goal here was *awareness* of these broader learning theory  

  - Spend more time talking about why this is useful for alignment  

  - People’s backgrounds are very different, and some people will read 2x slower
  than others \- this can sometimes mean a tradeoff between letting people slip
  behind or boring the most capable students. Not sure how to handle  

  - I (Zach Furman) am not an expert in Solomonoff induction but nevertheless
  ended up having to cover it, and my lack of knowledge was noticeable. Would
  have gone worse without other TAs there who knew more
---
## Overview
