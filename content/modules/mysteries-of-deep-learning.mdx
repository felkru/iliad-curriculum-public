...
teachingGuide: >-
  ...
  As with Day B.1, the goal is awareness and broad context, not deep technical
  mastery. The subsequent days (SLT, training dynamics, influence functions) go
  deep on specific approaches; this day ensures students know what those
  approaches are trying to explain


  The reading list is deliberately longer than can be comfortably covered in one
  day. The TA should select a subset based on audience interest and pacing. The
  "Deep Learning as Program Synthesis" post is the one reading that should not
  be cut, as it provides the overarching framework and connects back to Day B.1


  This day is reading-and-discussion-heavy rather than exercise-heavy. That's
  appropriate given the survey nature of the content, but it means discussion
