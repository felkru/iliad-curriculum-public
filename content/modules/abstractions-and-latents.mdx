...
Abstractions are the discrete, reusable concepts that agents use to compress and reason about the world. For alignment, the question is whether different agents — humans and AIs with different architectures and training — converge on the *same* abstractions, so that human values (themselves latent variables in human world models) can be translated into objectives an AI can pursue. This module surveys two formal frameworks for this question: **natural latents**, which characterise latents pinned down by mediation and redundancy, and **condensation**, which formalises how a world model organises into interpretable modular structure.
